    ...
    <header class="bg-white border-b border-gray-200 px-6 py-4">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-4">
                <!-- AI Status -->
                <div class="flex items-center space-x-2 px-3 py-2 bg-accent-50 rounded-lg">
                    <div :class="[
                        'w-2 h-2 rounded-full',
                        store.state.aiOnline ? 'bg-accent-500 animate-pulse' : 'bg-gray-400'
                    ]"></div>
                    <span :class="[
                        'text-sm font-medium',
                        store.state.aiOnline ? 'text-accent-700' : 'text-gray-600'
                    ]">
                ...
                <!-- Loading Indicator -->
                <div v-if="store.state.loading || store.state.generating" class="flex items-center space-x-2 px-3 py-2 bg-primary-50 rounded-lg">
                ...
                <div class="relative group">
                    <button class="w-8 h-8 bg-gradient-to-br from-primary-500 to-secondary-500 rounded-full flex items-center justify-center text-white font-medium hover:shadow-lg transition-shadow">
                        U
                    </button>
                    ...
                    <div class="absolute right-0 top-full mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                        <div class="p-2">
                            <button @click="exportData" class="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg">
                                <i class="fas fa-download mr-2"></i>Export Data
                            </button>
                            <button @click="showImportDialog" class="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg">
                                <i class="fas fa-upload mr-2"></i>Import Data
                            </button>
                            <hr class="my-2">
                            <button @click="resetData" class="w-full text-left px-3 py-2 text-sm text-red-600 hover:bg-red-50 rounded-lg">
        ...
        <div v-if="breadcrumbs.length > 0" class="flex items-center space-x-2 mt-4 text-sm">
            <button
                ...
                :class="[
                    'flex items-center space-x-1 px-2 py-1 rounded hover:bg-gray-100 transition-colors',
                    index === breadcrumbs.length - 1 ? 'text-gray-900 font-medium' : 'text-gray-600'
                ]"
            >
                <i v-if="crumb.icon" :class="crumb.icon" class="text-xs"></i>
                <span>{{ crumb.label }}</span>
                <i v-if="index < breadcrumbs.length - 1" class="fas fa-chevron-right text-gray-400 text-xs ml-1"></i>
            </button>
        </div>
    </header>
    `,
